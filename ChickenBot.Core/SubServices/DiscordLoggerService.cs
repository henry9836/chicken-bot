        ...
        private void RunLogSuppressions(LogEvent logEvent, ref LogEventLevel level)
        {
            ...
            switch (logEvent.Level)
            {
                ...
                case LogEventLevel.Information:
                    ...
                    if (template.Contains("Start processing HTTP request")
                        || template.Contains("Sending HTTP request")
                        || template.Contains("Received HTTP response headers after")
                        || template.Contains("End processing HTTP request after"))
                    {
                        level = LogEventLevel.Debug;
                    }
